# Especificación Funcional: Captura Inteligente de Inventarios (MVP)

**Feature Directory**: `specs/001-captura-inventarios`

**Feature**: `001-captura-inventarios`

**Created**: 2026-07-24

**Status**: Funcional — sin ambigüedades pendientes. Habilitada la fase de planeación técnica.

**Entrada**: `SPEC.md` (Especificación de Producto, Colsubsidio) + `.specify/memory/constitution.md` v1.0.0

> Este documento describe **qué** debe hacer el sistema y **por qué**, en lenguaje de negocio.
> No menciona lenguajes, marcos de trabajo, bases de datos ni estructura de código: eso corresponde a la fase de planeación, que no puede iniciar hasta que el checklist de este documento confirme cero ambigüedades (Constitución, Principio IX).

---

## 1. Contexto

Las bodegas de hoteles y parques de Colsubsidio cuentan su inventario físico en papel. El conteo se transcribe después al sistema central, y en esa cadena se pierden datos: se anota mal la cantidad, se confunde la unidad de medida, se registra un artículo que no es, o aparece mercancía que nadie sabe de dónde salió. El error no se detecta en la bodega —donde se puede verificar caminando tres metros— sino días después, cuando ya nadie recuerda qué había en el estante.

Este sistema reemplaza el papel por una captura asistida que valida **en el momento**, mientras la persona todavía está frente al producto, y que conserva la trazabilidad de quién contó qué y cuándo.
...
| Término | Significado |
|---|---|
| **Ronda** | El recorrido de **una persona** sobre **una bodega**. Unidad de aislamiento: las rondas no se pisan entre sí y se acumulan. |
| **Conteo ciego** | El Operador nunca conoce la cantidad que el sistema espera encontrar. |
| **Saldo esperado** | Cantidad que el sistema central registra para un artículo en una bodega. Invisible para el Operador. |
| **Discrepancia** | Diferencia entre lo contado y lo esperado que supera la tolerancia aplicable. |
| **Merma** | Diferencia tolerada para artículos medidos por peso. |
| **Producto fantasma** | Artículo hallado físicamente que no existe en el catálogo. |
| **Artículo conciliado** | Artículo cerrado sin reconteo del Auditor porque el conteo ciego coincidió con el saldo esperado dentro de la tolerancia aplicable. |
| **Ítem auditable** | Artículo que requiere decisión del Auditor. |
| **Consolidado** | Vista acumulada de todas las rondas de una bodega, con el detalle de cada una. |
| **Código de razón** | Causa registrada al cerrar una discrepancia, tomada de un catálogo controlado. |

---

## 3. Decisiones de negocio resueltas

Cinco decisiones que no estaban en `SPEC.md` y que cambian el alcance. Confirmadas por el negocio el **2026-07-24**. Se registran como decisiones —no como supuestos— porque fueron respondidas explícitamente.

### D1 · Cierre de ronda: captura libre con cuadre al cerrar

El Operador registra libremente lo que encuentra; no recorre una lista dirigida. Al cerrar su ronda, el sistema le presenta los artículos del catálogo que no registró y le exige decidir por cada uno: **contado en cero** o **no contado**.

Los dos estados son distintos y se conservan por separado. *Contado en cero* es una afirmación sobre la realidad física y activa la validación de discrepancia. *No contado* declara que el artículo quedó fuera del alcance de esa ronda.

### D2 · Rondas independientes, inmutables y acumulativas

- Cada persona trabaja en una **ronda propia y única**. Las rondas no se pisan entre sí, ni siquiera entre ejercicios distintos de la misma persona.
- Cada producto se cuenta por su **total** en la bodega, no por partes ni por ubicación.
- **Nada se actualiza en sitio.** Un registro no se sobrescribe: se supersede con uno nuevo. Sobrescribir destruiría la contabilidad de las rondas, que es justamente lo que se quiere conservar.
- Los históricos **se acumulan** y alimentan la comparación final.
- El Auditor recibe el **consolidado con todos los históricos**, no una cifra ya reconciliada.

### D3 · Identidad propia, sustituible por el directorio institucional

El sistema autentica contra un padrón de usuarios propio. La autenticación y la deducción de rol quedan aisladas de modo que sustituirlas por el directorio institucional de Colsubsidio no obligue a rehacer el resto del sistema.

### D4 · Doble registro en la misma ronda: es corrección

Si el Operador registra dos veces el mismo artículo dentro de su ronda abierta, el sistema advierte que ya fue contado, exige confirmación explícita, y el nuevo registro **supersede** al anterior como valor vigente de esa ronda. Ambos permanecen en la traza. **No se suman**: la cantidad declarada es siempre el total del producto (D2).

### D5 · La ceguera es la garantía — una ronda basta, el Auditor avala

> **Enmendada el 2026-07-25.** Reemplaza la versión anterior de D5 (*doble conteo obligatorio*), que queda **sin efecto**.

**Un artículo se confirma automáticamente cuando un conteo ciego coincide con el saldo esperado, dentro de la tolerancia de merma aplicable.**

El fundamento es la ceguera, no la repetición: el Operador no puede consultar el saldo esperado por ningún medio (FR-1.18, SC-1.3), así que **una coincidencia no se puede fabricar**. Contar bien es la única forma de producirla.

Todo lo demás llega al Auditor: los artículos con discrepancia contra el saldo esperado, los productos fantasma, los artículos que dos rondas contaron con cantidades distintas, y los artículos sobre los que ninguna ronda afirmó nada.

**Una sola ronda de conteo ciego, más la verificación del Auditor, es suficiente para dar el aval.** Habrá bodegas donde se ejecuten dos o más rondas; el sistema **DEBE** soportarlo, pero **NO** lo exige ni lo convierte en condición de cierre.

*Independencia:* el Auditor no cuenta el inventario, lo verifica, y pertenece a un equipo independiente de quien contó — interno o incluso externo. Por eso ve toda la información, incluida la diferencia contra el saldo esperado: de su lado no hay conflicto de interés que proteger con ceguera.

*Consecuencia de alcance:* el inventario de una bodega se ejecuta con **un** recorrido. La versión anterior exigía dos recorridos completos con personas distintas —duplicando la mano de obra del inventario— a cambio de cubrir un caso remoto: que un conteo equivocado coincidiera exactamente con un saldo esperado también equivocado.

---

### D6 · Trabajo dirigido — ⛔ REVOCADA el 2026-07-25, sin efecto

> **Esta decisión fue revocada por el negocio.** Se conserva por trazabilidad: rigió entre el 2026-07-24 y el 2026-07-25. **La decisión vigente es D7**, que restablece la captura libre de D1. Nada de este apartado se implementa.

**Fecha: 2026-07-24 (posterior a la aprobación de D1).**

El sistema **conduce** al Operador artículo por artículo en vez de esperar a que él enuncie qué está contando. En cada turno el sistema dice qué artículo toca, el Operador responde solo la cantidad y la unidad, y el sistema confirma por voz antes de pasar al siguiente.

**Qué cambia respecto de D1:**

| | D1 (captura libre) | D6 (trabajo dirigido) |
|---|---|---|
| Quién nombra el artículo | El Operador | El sistema |
| Turno de voz | "arroz blanco, veinte kilogramos" | Sistema: "arroz blanco" → Operador: "veinte kilos" |
| Desambiguación de artículo | Necesaria en cada turno | Innecesaria: el artículo ya está determinado |
| Cuadre al cierre | Sobre lo que el Operador no tocó | Sobre lo que el Operador saltó explícitamente |

**Qué se conserva de D1:** el Operador **puede saltar** un artículo y seguir; el cuadre al cierre sigue existiendo y sigue exigiendo la decisión entre *contado en cero* y *no contado* por cada artículo no resuelto. Lo que desaparece es la obligación de que el Operador identifique el artículo por voz.

**Fundamento:** elimina la clase de error más cara del flujo — resolver de qué artículo se habla — porque en un turno dirigido el artículo no es una inferencia sino un dato. Lo que queda por interpretar es una cantidad y una unidad, que una gramática determinista resuelve sin modelo de lenguaje.

**Consecuencia sobre requisitos ya escritos:** *(anulada por D7).*

---

### D7 · Captura libre con resolución determinista del nombre — ⭐ VIGENTE, restablece D1 y revoca D6

**Fecha: 2026-07-25.** Es el flujo definitivo del Operador.

El Operador **dicta lo que ve**. El sistema no lo conduce: escucha, resuelve contra el catálogo y pregunta cuando duda.

**El turno completo, paso a paso:**

1. El Operador entra a la web e **inicia el agente de voz**. A partir de ahí habla de corrido.
2. Enuncia **nombre del producto, cantidad y unidad** — *"platos cuadrados, tres unidades"*.
3. El sistema **resuelve el nombre contra el catálogo de la bodega** por similitud de texto.
   - Si lo resuelve con certeza → **lo confirma y guarda**. Es, literalmente, dictarle.
   - Si no → **muestra en pantalla los nombres sugeridos** y el Operador toca el correcto. El sistema **nunca elige por su cuenta**.
4. En paralelo, el sistema **compara contra la base de datos**: nombre, cantidad y unidad de medida contra el inventario original.
5. Si la cantidad resulta incoherente → **lanza una alerta que es una pregunta**: *"¿estás seguro de que contaste X?"*. **Jamás muestra el número de la base, ni como valor por defecto, ni como rango, ni como pista.** La alerta existe para que el Operador verifique lo que él mismo dijo, no para acercarlo a una cifra.
6. Si el Operador confirma su cantidad → **el sistema la guarda con una nota de auditoría**. No la descarta ni la corrige.
7. Si la diferencia es real → **se reporta para revisión del Auditor**.

**Qué NO hace el sistema:** no dicta la lista, no impone un orden, no elige un artículo ambiguo, y no revela nunca el saldo esperado.

**Fundamento del negocio:** el Operador conoce su bodega y su recorrido (G-7). Conducirlo artículo por artículo por un catálogo de miles de referencias —la mayoría de las cuales no están físicamente en esa bodega— es más lento que dejarlo enunciar lo que tiene delante. La desambiguación de nombre, que D6 quería eliminar, se resuelve de forma **determinista** con búsqueda por similitud sobre un catálogo cerrado y, cuando empata, con una pregunta en pantalla. No hace falta un modelo de lenguaje para eso.

**Consecuencia sobre requisitos ya escritos:** FR-1.11 (candidatos ante ambigüedad) **vuelve a ser flujo normal** y deja de estar acotado a productos fantasma. La Historia 1 se reescribió a este flujo el 2026-07-25. Los requisitos FR-1.22 a FR-1.25 se reescribieron: donde exigían conducir al Operador, ahora exigen **resolver el nombre de forma determinista** y prohíben expresamente dictarle qué contar. Y desaparece el orden aprendido de recorrido: el sistema ya no propone artículos, así que no necesita saber en qué orden se recorre la bodega.

---

### D8 · La base madre no se sobrescribe: los conteos son registros hijos

**Fecha: 2026-07-25.**

El conteo de un Operador **nunca modifica el inventario original**. Por cada artículo contado se crea un **registro hijo, heredado de la tabla madre**, que conserva juntos **el número inicial del sistema y el número contado**. Todos los conteos de todos los Operadores se acumulan así, sin pisar nada ni pisarse entre sí.

**El inventario original se actualiza únicamente cuando el Auditor da el aval.** Hasta ese momento el sistema acumula evidencia; no modifica la fuente.

*Relación con D2:* D2 ya prohibía actualizar en sitio dentro del libro de conteos. D8 extiende la misma regla hacia arriba: tampoco se toca la tabla madre. La consecuencia es que en todo momento se puede responder "¿qué decía el sistema y qué encontró la gente?" sin reconstruir nada.

---

## 4. Historias de usuario

Ordenadas por prioridad. Cada una es desarrollable, probable y demostrable de forma independiente.
...
- **Usuario**: persona que opera el sistema. Identificación, credenciales, rol, bodegas habilitadas.
- **Rol**: Operador, Auditor o Administrador. Determina qué puede hacer; se deduce, no se elige.
- **Bodega**: sitio físico donde se cuenta. Concepto genérico, no atado a hoteles ni parques.
- **Artículo de catálogo**: producto conocido por el sistema central. Nombre (identificador principal), código cuando existe, unidad de medida esperada, si se mide por peso.
- **Saldo esperado**: cantidad que el sistema central registra para un artículo en una bodega. Nunca visible para el Operador.
- **Ronda**: recorrido de una persona sobre una bodega. Estado (abierta, cerrada), autor, momentos de apertura y cierre.
- **Registro de conteo**: lo que una persona registró de un artículo dentro de una ronda, como cantidad total. Inmutable. Estado (contado, contado en cero, no contado), cantidad, unidad, modo de captura, autor, momento, clave de idempotencia, marcas de advertencia y evidencia, y si fue superseded.
- **Producto no registrado (fantasma)**: hallazgo sin correspondencia en el catálogo. Descripción detallada, unidad observada, cantidad, autor.
- **Discrepancia**: diferencia entre lo contado y lo esperado que supera la tolerancia. Estado (abierta, en reconteo, cerrada) y código de razón al cerrarse.
- **Consolidado de bodega**: vista acumulada de todas las rondas cerradas, con la clasificación de cada artículo en conciliado o auditable.
- **Reconteo**: registro del Auditor sobre un ítem auditable. Prevalece sobre el de los Operadores.
- **Código de razón**: causa de un cierre de discrepancia, de un catálogo controlado.
- **Configuración de merma**: tolerancia por unidad de medida, con su historial de cambios.
- **Constancia de salida**: registro de qué consolidado se exportó o envió, en qué formato o hacia dónde, cuándo y por quién.

---

## 6. Fuera de alcance del MVP

Aplica a todo el producto, además de lo excluido en cada historia.

- Aplicación móvil nativa.
- Lectura de código de barras o RFID.
- Operación desconectada durante jornadas completas.
- Alta de productos fantasma en el catálogo maestro del sistema central.
- Gestión de órdenes de compra, traslados entre bodegas y ajustes contables.
- Resolución automática de contradicciones entre rondas: el sistema las expone, el Auditor decide.
- Administración de usuarios, roles y contraseñas desde el sistema.
- Tableros analíticos e informes históricos más allá del consolidado de un inventario.

---

## 7. Dependencias globales

- Catálogo de artículos por bodega, con unidad de medida esperada, provisto por el sistema central.
- Saldos esperados por bodega y artículo.
- Padrón de usuarios con rol asignado.
- Catálogo de códigos de razón, definido por el negocio.
- Valores iniciales de tolerancia de merma.
- Acceso a la vía de integración del sistema central de inventarios.
- Dispositivos móviles de la compañía disponibles para prueba en bodega real.

---

## 8. Supuestos globales

- **G-1**: El catálogo y los saldos provienen del sistema central; esta aplicación los consume y no los modifica.
- **G-2**: La red de las bodegas es intermitente, no ausente.
- **G-3**: Los dispositivos son teléfonos y tabletas de la compañía con navegador moderno.
- **G-4**: El negocio confirmará los criterios objetivos de "descripción detallada" para productos fantasma.
- **G-5**: El 0,2% de merma citado en `SPEC.md` §4.2 es un ejemplo ilustrativo, no un valor confirmado.
- **G-6**: No se declara una meta de reducción porcentual frente al proceso en papel porque no existe línea base medida del proceso actual. Debe medirse antes de comprometerla.
- **G-7**: **La disposición física de las bodegas es estable y el recorrido de conteo es siempre el mismo.** Los productos no cambian de lugar y los operarios repiten la misma ruta, que ya conocen. Confirmado por el negocio el 2026-07-25. Consecuencia: el sistema **no necesita ningún dato de ubicación** ni orden de recorrido — el Operador sabe por dónde va y dicta lo que ve (D7). El sistema no propone artículos, así que tampoco tiene que ordenarlos.
